...
filename='Result_3segments_Point'
...
class Maze:
    ...
    def is_EndOf_maze(self, client):
		if Dist.euclidean(client.ToolPosition[0][0:3], [self.x, self.y, self.z]) < 50:    
		    return True
		else:	
			return False
			
    def is_Testpoint(self, client):
		if Dist.euclidean(client.ToolPosition[0][0:2], [self.x, self.y]) < 50: 
			return True
		else:
			return False
...
if __name__ == '__main__':
    ...
    Test_point = Maze(585.9,-192,153) # point No:2
    Test_point2 = Maze(648,91.6,165) # point No:3
    ...
    Proba.append(user_n+"_proba")
    Trust.append(user_n+"_trust")
    y_prd .append(user_n+"_pred")
    ...
    while True:
		...
		if End_point.is_EndOf_maze(my_client):
			...
			profile=Create_profile_For_TestData(Data,filename,Present_task)
			Y_pred, Y_prob= Authentication('profile_3Segment_2_19_50.csv',profile)
			print 'Y_prob',Y_prob
		...
		if Test_point.is_Testpoint(my_client):
			print ('---------------------------------------------------')
			print ('Test point1 ')
			print len(Data)
			print ('---------------------------------------------------')
		...
		elif Test_point2.is_Testpoint(my_client):
			print ('---------------------------------------------------')
			print ('Test point 2')
			print len(Data)
			print ('---------------------------------------------------')
